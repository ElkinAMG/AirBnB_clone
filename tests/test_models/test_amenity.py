...
class StateTest(unittest.TestCase):
    ...
    def test_doc_string(self):
        ...
        self.assertEqual(len(Amenity.__doc__) > 0, True)
        self.assertEqual(len(Amenity.__init__.__doc__) > 0, True)
        self.assertEqual(len(Amenity.__str__.__doc__) > 0, True)
        self.assertEqual(len(Amenity.save.__doc__) > 0, True)
        self.assertEqual(len(Amenity.to_dict.__doc__) > 0, True)

    def test_instance(self):
        """testing to see whether created instance is equal to State"""
        self.assertTrue(isinstance(self.ins, Amenity))
        self.assertTrue(self.ins.__class__ == Amenity)
